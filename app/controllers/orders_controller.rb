class OrdersController < ApplicationController
  ...
  def create
    ...
    ActiveRecord::Base.transaction do
      if @order.save
        inventory_service = InventoryService.new(@order, @pizza, side_item, toppings)
        inventory_service.update_stock
        order_service = OrderService.new(@order, @pizza, side_item, toppings, order_params[:quantity])
        price = order_service.calculate_price
